...
# use the stressball for deformable interaction
# map to game controls (to top-down game, similar perspective to looking at waterbath)
    # roll ball up = walk up; etc.
    # squish = jump / shoot
    # squash = crouch / take down
# -> HM clone type game

"""
TODO
- [ ] detect ball outline
- [ ] differentiate between directions and squish/squash
    - move: find center of circle and move into that direction (get most left/up/... contour)
        - alt: use biggest contour as the mover
    - squish: find distance between closest points -> if under threshold then yes
    - squash: find distance between closest points -> if over threshold then yes
    -> weight contours in top half more, than bottom half (here more likely to be artifacts/bubbles, acoustic "shadowing")
"""

"""
NOTE: Log
- increase contrast and brightness: get rid of the background (and by that also some noise)
- blur : smoothes images 
    - frame = cv2.GaussianBlur(frame, (7,7), 0) 
- dilate/erode: ??? (schaut alles gleich aus)
    - frame = cv2.erode(gray, (21, 21))
    - frame = cv2.dilate(frame, (21, 21))
- simplify contours:
...
class Deformer(Demo):
    ...
    def do(self, frame: MatLike, gray_cut: MatLike) -> MatLike:
        ...
        q = do_color_quantization(gray_cut, gray_cut, 4)
